...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        # instance of Schedule Linear class
        sl = ScheduleLinear(anneal_timesteps, epsilon_final, epsilon_initial)
        # get epsilon with our Schedule Linear method
    ...
    def q_learning(self):
        ...
        while episode <= self.episode_max and diff > self.threshold:
            ...
            while not end_episode:
                ...
                Q[s_current][action] = (1 - lr) * Q[s_current][action] + lr * (R + self.gamma * max(Q[s_next]))
                # ADD YOUR CODE SNIPPET BETWEEN EX. 2.2
            ...
            diff = abs(np.mean(Q) - np.mean(Q_old))
            # ADD YOUR CODE SNIPPET BETWEEN EX. 2.3
            Q_old[:] = Q
            print(
...
class ScheduleLinear(object):
    ...
    def value(self, t):
        ...
        if t < self.schedule_timesteps:
            ...
            eps_t = self.initial_p + delta_eps * (t / self.schedule_timesteps)
            return eps_t
        else:
            return self.final_p
